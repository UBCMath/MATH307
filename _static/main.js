$(function() {
  $("div.bigidea > p:first-child").prepend("<strong>Big Ideas. </strong>");
  $("div.definition > p:first-child").prepend("<strong>Definition. </strong>");
  $("div.example > p:first-child").prepend("<strong>Example. </strong>");
  $("div.theorem > p:first-child").prepend("<strong>Theorem. </strong>");
  $("div.proposition > p:first-child").prepend("<strong>Proposition. </strong>");
  $("div.note > p:first-child").prepend("<strong>Note. </strong>");
});
